...
public class ConfigureSimulationPanel extends JPanel 
	...
	private void initUI(){
		...
		tabbedPane.addTab("Fog node Configuration", createDcTab());
	...
	private JPanel createDcTab(){
		...
		compW = 70; 
		compH = 40;
		JLabel lblDcHeading = new JLabel("<html>Fog<br/>Node:</html>");
	...
	private JPanel createAdvancedTab(){
		...
		compW = 260;
		lastCompH = compH = 70;
		JLabel lblDcRequestGrouping = new JLabel("<html>Request grouping factor in Fog nodes:" +
				                                  "<br/>(Equivalent to number of simultaneous" +
		...
		JLabel lblLoadBalancing = new JLabel("<html>Load balancing policy<br/>" +
				                                  "across VM's in a single Fog node:</html>");
		...
		cmbLoadBalancingPolicy = new JComboBox(new String[]{
				...
				Constants.Load_Balancing_HEFT,
				Constants.Load_Balancing_SJF,
				Constants.Load_Balancing_SJFB
		});
	...
	private JPanel createDcDetailsPanel(){
		...
		JLabel header = new JLabel("Physical Hardware Details of Fog node :");
	...
	private void addVmAllocation() {
		if (dataCenterList.size() == 0){
			JOptionPane.showMessageDialog(this, "Please create at least one Fog node before you" +
