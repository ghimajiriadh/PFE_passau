...
public class DCNSFog {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*else{
				for(FogDevice device : fogDevices){
					if(device.getName().startsWith("d")){ // names of all Smart Cameras start with 'm'
			...
			controller.submitApplication(application, 
					(CLOUD)?(new ModulePlacementMapping(fogDevices, application, moduleMapping))
							:(new FCFS(fogDevices, sensors, actuators, application, moduleMapping,numOfAreas,numOfCamerasPerArea)));
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		FogDevice router = createFogDevice("d-"+id, 2700, 4000, 10000, 10000, 2, 0.0, 107.339, 83.4333);
		//FogDevice router1 = createFogDevice("d1-"+id, 1800, 4000, 10000, 10000, 2, 0.0, 107.339, 83.4333);

		fogDevices.add(router);
		//fogDevices.add(router1);
		router.setUplinkLatency(2); // latency of connection between router and proxy server is 2 ms
		//router1.setUplinkLatency(2);
		...
		//router.setParentId(router1.getId());
		router.setParentId(parentId);
		List childList = new ArrayList();
		//childList.add(router.getId());
		//router1.setChildrenIds(childList);
		//System.out.print(router1.getChildrenIds().size());
		return router;
	}
	
	private static FogDevice addCamera(String id, int userId, String appId, int parentId){
		FogDevice camera = createFogDevice("m-"+id, 500, 1000, 10000, 10000, 4, 0, 87.53, 82.44);
	...
	private static Application createApplication(String appId, int userId){
		 ...
		 */
		application.addAppEdge("CAMERA", "motion_detector", 1000, 10000, "CAMERA", Tuple.UP, AppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of type CAMERA
