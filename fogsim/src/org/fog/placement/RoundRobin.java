...
public class RoundRobin extends ModulePlacement{
    ...
    public RoundRobin(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators,
    ...
    protected void mapModules() {
        ...
        while(!nonPlacedModules.isEmpty())
        {   nb=nbresensors;
            AppModule modToPlace= nonPlacedModules.iterator().next();
       // System.out.println(modToPlace.getName());
            while(nb>0){
            ...
            nonPlacedModules.remove(modToPlace);
    ...
    private void placeMod(List<FogDevice>routers,AppModule apM,Double CpuLoad)
    {
        for (FogDevice dev : routers)
        {
            ...
            if (CpuLoad+getCurrentCpuLoad().get(dev.getId())<dev.getHost().getTotalMips()-1400)
            {
